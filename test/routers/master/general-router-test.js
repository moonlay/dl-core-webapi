...
it('#01. Should be able to get list', function (done) {
    request(uri)
        .get('/v1/master/products/generals')
        .expect(200)
...
it('#02. should success when create new data', function (done) {
    var data = getData();
    request(uri).post('/v1/master/products/generals')
        .send(data)
...
it(`#03. should success when update created data`, function (done) {
    request(uri).put('/v1/master/products/generals')
...
it("#04. should success when delete data", function (done) {
    request(uri).del('/v1/master/products/generals/:id')
        .query({ _id: createdId })
        .end(function (err, res) {
            if (err) {
                done(err);
